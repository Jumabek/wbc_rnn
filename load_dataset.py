...
def def_read_images(dir):
	...
	for path, subdirs, files in os.walk(dir):
		...
		for filename in files:
			...
			img = np.asarray(img).reshape(-1) # curent shape=128x128x3 -1 means variableness
...
def def_load_images(directory):
	...
	return img_arr,labels_index

def main(data_dir ):
	# print('main function is called!!!')
	print('mizno, load data set')

	images,labels = def_load_images(data_dir)
	...
	#data_x = images[:,0] # image data
	#data_y = images[:,1] # label
	
	print('mizno, training data x (image data) = ' + str(data_x.shape))
	print('mizno, training data y (label) = ' + str(data_y.shape))
